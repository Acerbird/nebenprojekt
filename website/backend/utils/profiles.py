...
def timestamps(hours: int, season: str) -> List[str]:
    """ISO-Zeitstempel, beginnend Montag 00:00 der jeweiligen Jahreszeit."""
    start = datetime.strptime(season_config(season)["start"], "%Y-%m-%d")
    return [(start + timedelta(hours=h)).isoformat(timespec="minutes")
            for h in range(hours)]


def load_series(hours: int, peak_gw: float, season: str) -> List[float]:
    """Stündliche Last in GW."""
...
def solar_series(hours: int, installed_gw: float, season: str) -> List[float]:
    """Stündliche PV-Einspeisung in GW (glatter Tagesgang, keine Wolken)."""
    cfg = season_config(season)
    start = datetime.strptime(cfg["start"], "%Y-%m-%d")
...
def wind_series(hours: int, installed_gw: float, season: str, seed: int = 7) -> List[float]:
    ...
    for _ in range(hours):
        ...
        out.append(round(installed_gw * min(max(cf, 0.0), 0.92), 3))
    return out
